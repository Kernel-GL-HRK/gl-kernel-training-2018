			...
			const char *buf, size_t count)
{
	...
	while (s) {
		...
		slabbuf = mempool_alloc(mypool, GFP_KERNEL);
		if (slabbuf == NULL) {
...
static int __init omod_init(void)
{
	...
	pr_info("omod7 SysFS ToLower v4(mempool) startup...\n");
	...
	mypool = mempool_create(MYPOOL_MIN_RESERVE,
		mempool_alloc_slab, mempool_free_slab, pmycache);
	...
	pr_info("omod7 SysFS ToLower(mempool) module loaded.\n");
	return 0;
}
...
static void __exit omod_exit(void)
{
	...
	pr_info("omod7 SysFS ToLower(mempool) normal shutdown.\n");
}
