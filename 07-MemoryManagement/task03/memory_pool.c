...
#define NUM_OF_PAGES_IN_POOL 16
...
static mempool_t         *msg_memory_pool;
...
static int msg_pool_create(void)
{
	...
	msg_memory_pool = mempool_create_slab_pool(NUM_OF_PAGES_IN_POOL,
		msg_cache_ptr);
	return msg_cache_ptr ? 0 : -EFAULT;
}

static void msg_pool_destroy(void)
{
	mempool_destroy(msg_memory_pool);
...
static void msg_list_free(void)
{
	...
	list_for_each_entry_safe(ptr, tmp, &msg.ptr, ptr) {
		...
		mempool_free(ptr, msg_memory_pool);
	}
}
	...
	const char *buf, size_t count)
{
	...
	//tmp =  mempool_alloc_slab(GFP_KERNEL, msg_cache_ptr);
	tmp = mempool_alloc(msg_memory_pool, GFP_KERNEL);

	pr_info("memory_slab_list: #%d in pool ", msg_memory_pool->curr_nr);
